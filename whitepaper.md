...
Progress for the LGBT community has become visible through landmark events, such as:
  ...
  * the emergence of publicly open LGBT politicians in senior leadership roles, such as  Jóhanna Sigurðardóttir, the Prime Minister of Iceland, in January 2009 and Leo Varadkar, the Irish Taoiseach, as of June 2017<sup>int-3</sup>;
...
> ** The LGBT Foundation to issue and govern the LGBT Token.

The LGBT Token is a cryptographic asset that follows the ERC-20 standard <sup>token-1</sup> for smart contracts running on the public Ethereum blockchain. 
...
3. Much faster and cheaper transactions - enabling micropayments that are prohibitive with legacy payment infrastructure 

4. Encrypted identity on the blockchain ensures privacy while simultaneously guaranteeing you can cryptographically trust who you are talking to through asymmetric encryption<sup>token-2</sup>
...
Vitalik Buterin, Ethereum Founder said in a tweet<sup>token-4</sup> on 12 December 2017, "
...
There are a number of game-theoretic reasons why PoS is preferable to PoW. There are "economic incentives to discourage centralized cartels and anti-centralization incentives to discourage cartels from forming in the first place"<sup>token-3</sup>.

The Proof-of-Stake idea can be extended to include masternodes that can manage off-chain transactions that use Merkle trees to sync cross-node state <sup>token-5</sup>. Masternodes fulfill the same critical role as miners: they secure the network. Retailers that accept LGBT-Token will run LGBT-Masternodes to get near-instant block confirmations with a minimum required stake that is linked to your reputation as a sort of deposit against attempting to double spend or otherwise engage in malicious activity towards the network. Users and non-merchants can themselves run masternodes if they would like put up staking tokens in exchange for earning fees to secure the network. The LGBT-Foundation will run nodes to match at least 20% of MasterNodes in the network. 
...
Furthermore, Hornet pledges to migrate its existing Reward program for community contributions from current "Perk" to LGBT Token and will be exploring other novel applications of LGBT-token in the app (e.g. boosting profiles while users stake LGBT, rewarding users for content by sharing advertising revenue denominated in LGBT Tokens, etc.)

> Alex: I added some interesting example use cases. I used vague language to be non-commital but I definitely think we should put forward some exciting use cases. Again, people reading this whitepaper are going to be skeptical and wondering why we need a new token. Aligning with the LGBT community and giving them their own currency is compelling but there are actual legitimate applications of a TOKEN that we should mention to get the reader excited about the technology and potential applications that would not be possible WITHOUT a token. 
...
>**ALEX: I added some detail on the reserve - please confirm my understanding of this is correct - the idea is that we're minting the reserve until the cap is reached? that seems to be standard practice?**

>**ALEX: I also think we should mention what happens if the full pre-sale amount is not sold. The common practice is to burn tokens allocated to pre-sale that are not sold. see page 16 of the allpublicart.io whitepaper **
...
Team Tokens will be used used to fund the development of the platform and incentive current and future contributors to the LGBT Foundation. Denominating development funding in tokens aligns incentives. All Team Tokens are subject to a lock-up period of 12 months and are vested subject to hitting project milestones and goals.

[alex note: I made some wording changes here - I think it's important for the wording to not talk about rewarding us for past work but rather incentivize us for future work. I'd suggest explicitly stating that the tokens are issued to the team to "align incentives".]
...
>**ALEX: We should discuss how the code itself is updated with respect to the foundation or token holders. See this from the GDAX Digital Asset Framework - the manual they use for whether to list a token on CoinBase / Gdax - we need to tick all these boxes to get listed by major exchangs: There is a structured process to propose and implement major updates to the code, or there is a system or voting process for conflict resolution.**
...
> Alex: I mention on-chain voting for distribution of LGBT funds. I think we should consider adding that above. Can keep it vague like "R&D on implementing distributed voting for how to allocate LGBT Foundation grants."
...
## Footnotes
  ...
  * token-2: https://www.princeton.edu/~rblee/ELE572Papers/CSurveys_SymmAsymEncrypt-simmons.pdf
  * token-3: https://medium.com/@VitalikButerin/a-proof-of-stake-design-philosophy-506585978d51
  * token-4: https://twitter.com/VitalikButerin/status/940429788673536000
  * token-5: Merkle, R.C. "Method of providing digital signatures", 1982, US Patent 4,309,569
}
  ...
  * goals-1: Or at least not in a decentralized manner where the control of the data lays in the hands of the individual member. Recent hacks and data leaks (from Equifax all the way to Ashley Madison) showed how flawed centralized storage and control of sensitive data can be.
